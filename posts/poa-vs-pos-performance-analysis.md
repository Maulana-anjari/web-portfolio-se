...
Before exploring the metrics, it is vital to understand the fundamental difference in how these two mechanisms achieve consensus.
...
1.  **PoA Network:** I configured it using the *Clique* consensus engine.
2.  **PoS Network:** I configured it using the *Prysm* consensus client.
...
1.  **Latency & Responsiveness:** PoA achieves lower latency (4.6s vs 8.0s). Because validators are predefined and implicitly trusted, blocks can be sealed almost instantly. In contrast, PoS requires network-wide voting and epoch finalization algorithms, introducing deliberate delay to ensure global consistency.
2.  **Throughput vs. Scale:** The PoS network achieves higher overall throughput capacity (≈60 TPS compared to PoA's 40 TPS) under stress. However, for a system like DChain designed for user-interactive academic services (like issuing certificates), PoA's lower CPU overhead and rapid responsiveness provide more value than raw throughput.

## Conclusion

The right consensus mechanism depends on the required context. 
