...
The platform is currently in the design and specification phase. I produced a complete Product Requirements Document (PRD v2.0), a Technical Architecture Document (TAD v2.0), and a comprehensive Design System (FITTED.DESIGN.md). No production code has been deployed; the work to date has focused on system design, architecture decisions, and product definition.
...
Fitted orchestrates existing foundation models:
...
**Private-by-default storage with scoped sharing.** All storage buckets are private with Row Level Security. The system generates public URLs only for explicit share actions. This is more complex than a fully public bucket but necessary for user trust and compliance with Indonesian data protection regulations (PDP).
...
Designing an AI-powered consumer product from scratch required weighing trade-offs across multiple dimensions simultaneously: infrastructure cost vs. control and development speed vs. scalability. The most valuable discipline was documenting explicit non-goals (no microservices, no Kubernetes, no custom ML, no ORM) to prevent scope creep during design.

The Supabase-native architecture taught me that "no backend" still requires architecture. Edge Functions need careful prompt engineering and error handling, especially around timeout management. The migration path from serverless to dedicated backend needs to be designed upfront, even if the trigger conditions (10k MAU, 100k AI requests/month) are distant. Retrofitting a backend layer into a tightly coupled Supabase app would be expensive.

The design system work reinforced that AI products need especially careful UX vocabulary. The decision to use "Purple = AI Intelligence" as a learned color cue, and to ban the word "outfit" in favor of "Fit" throughout the codebase and UI, was a deliberate attempt to build a distinct product identity. Whether this vocabulary constraint survives user testing remains an open question worth measuring.
